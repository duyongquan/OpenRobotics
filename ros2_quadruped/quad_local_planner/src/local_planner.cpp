...
void LocalPlanner::initLocalBodyPlanner() 
{
    ...
    quad_nmpc_controller::SystemID type;

    if (robot_name_ == "spirit") {
        type = quad_nmpc_controller::SPIRIT;
    } else if (robot_name_ == "a1") {
        type = quad_nmpc_controller::A1;
    } else {
        RCLCPP_WARN(this->get_logger(), "WRONG ROBOT TYPE");
    }
...
void LocalPlanner::robotPlanCallback(const quad_msgs::msg::RobotPlan::SharedPtr msg) 
{
    body_plan_msg_ = msg;
}

void LocalPlanner::robotStateCallback(const quad_msgs::msg::RobotState::SharedPtr msg) 
{
...
void LocalPlanner::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg) 
{
    // Ignore non-planar components of desired twist
    cmd_vel_[0] = (1 - cmd_vel_filter_const_) * cmd_vel_[0] + cmd_vel_filter_const_ * cmd_vel_scale_ * msg->linear.x;
    cmd_vel_[1] = (1 - cmd_vel_filter_const_) * cmd_vel_[1] + cmd_vel_filter_const_ * cmd_vel_scale_ * msg->linear.y;
    cmd_vel_[2] = 0;
    cmd_vel_[3] = 0;
    cmd_vel_[4] = 0;
    cmd_vel_[5] = (1 - cmd_vel_filter_const_) * cmd_vel_[5] + cmd_vel_filter_const_ * cmd_vel_scale_ * msg->angular.z;

    // Record when this was last reached for safety
    last_cmd_vel_msg_time_ = rclcpp::Clock().now();
}
...
void LocalPlanner::getReference() 
{
    if (first_plan_) 
    {
        ...
        if (use_twist_input_) {
            // initial_timestamp_ = rclcpp::Clock()::now() - rclcpp::Duration(1e-6);
        }
    }
    ...
    local_footstep_planner_->getFootPositionsBodyFrame(
        current_state_, current_foot_positions_world_, current_foot_positions_body_);
    ...
    if (use_twist_input_) 
    {
        ...
        // rclcpp::Duration time_elapsed_since_msg =  rclcpp::Clock().now(); - last_cmd_vel_msg_time_;
        // if (time_elapsed_since_msg.seconds() > last_cmd_vel_msg_time_max_) 
        // {
        //     cmd_vel_.setZero();
        //     RCLCPP_INRO(this->get_logger(), 1.0, "No cmd_vel data, setting twist cmd_vel to zero");
        // }
        ...
        for (int i = 0; i < num_feet_; i++) 
        {
            support_center.x() += robot_state_msg_->feet.feet[i].position.x / ((double)num_feet_);
            support_center.y() += robot_state_msg_->feet.feet[i].position.y / ((double)num_feet_);
        }
        ...
        bool is_stepping =
            ...
            (support_center - current_state_.segment(0, 2)).norm() > stand_pos_error_threshold_);
        ...
        if (is_stepping) {
            ...
            current_stand_pose << support_center[0], support_center[1], current_state_[5];
        ...
        if ((current_state_ - ref_body_plan_.bottomRows(1).transpose()).norm() <= stand_pos_error_threshold_) {
            control_mode_ = STAND;
        }
    }
...
void LocalPlanner::publishLocalPlan() 
{
    ...
    // local_body_planner_nonlinear_->getNLPDiagnostics().loadDiagnosticsMsg(local_plan_msg.diagnostics);
    ...
    for (int i = 0; i < N_current_ - 1; i++) 
    {
        ...
        //                              rclcpp::Duration(first_element_duration_) + rclcpp::Duration((i - 1) * dt_);
    ...
    // auto t_publish = rclcpp::Clock()::now();

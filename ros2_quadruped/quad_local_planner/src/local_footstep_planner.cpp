...
void LocalFootstepPlanner::updateMap(const quad_utils::FastTerrainMap &terrain) 
{
    terrain_ = terrain;
}
    ...
    Eigen::VectorXd &foot_positions_body) 
{
    for (int i = 0; i < num_feet_; i++) {
        foot_positions_body.segment<3>(3 * i) = foot_positions_world.segment<3>(3 * i) - body_plan.segment<3>(0);
    }
}
    ...
    Eigen::MatrixXd &foot_positions_body) 
{
    ...
    for (int i = 0; i < horizon_length_; i++) {
        foot_pos.setZero();
        getFootPositionsBodyFrame(body_plan.row(i), foot_positions_world.row(i), foot_pos);

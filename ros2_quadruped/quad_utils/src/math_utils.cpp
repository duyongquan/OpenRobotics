    ...
    const double query_point) 
{
  // Check bounds, throw an error if invalid since this shouldn't ever happen
    ...
    for (long unsigned int i = 0; i < input_vec.size(); i++) {
    ...
    for (long unsigned int i = 0; i < input_mat.front().size(); i++) {
    ...
    const double query_point) 
{
    ...
    // Apply linear interpolation for each element in the vector
    for (long unsigned int i = 0; i < output_mat.front().size(); i++) {
...
int interpInt(const std::vector<double> input_vec, std::vector<int> output_vec, const double query_point) 
{
    // Check bounds, throw an error if invalid since this shouldn't ever happen
    if ((query_point < input_vec.front()) || (query_point > input_vec.back())) {
        throw std::runtime_error("Tried to interp out of bounds");
...
std::vector<double> centralDiff(std::vector<double> data, double dt) 
{
    ...
    for (auto i = 0; i < data.size(); i++) {  // NOLINT
...
std::vector<double> unwrap(std::vector<double> data) 
{
    ...
    for (long unsigned int i = 1; i < data.size(); i++) {
        ...
        if (diff > M_PI) {
            for (long unsigned int j = i; j < data.size(); j++) {

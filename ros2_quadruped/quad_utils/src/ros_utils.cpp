...
void updateStateHeaders(quad_msgs::msg::RobotState &msg, rclcpp::Time stamp, std::string frame, int traj_index) 
{
    ...
    double t_interp, std_msgs::msg::Header &interp_header) 
{
    ...
    // ros::Duration interp_duration =
    //     ros::Duration(t_interp * state_duration.toSec());
                    ...
                    double t_interp, quad_msgs::msg::BodyState &interp_state) 
{
    ...
    interp_state.pose.position.x = math_utils::lerp(state_1.pose.position.x, state_2.pose.position.x, t_interp);
    interp_state.pose.position.y = math_utils::lerp(state_1.pose.position.y, state_2.pose.position.y, t_interp);
    interp_state.pose.position.z = math_utils::lerp(state_1.pose.position.z, state_2.pose.position.z, t_interp);
    ...
    // Interp twist
    interp_state.twist.linear.x = math_utils::lerp(state_1.twist.linear.x, state_2.twist.linear.x, t_interp);
    interp_state.twist.linear.y = math_utils::lerp(state_1.twist.linear.y, state_2.twist.linear.y, t_interp);
    interp_state.twist.linear.z = math_utils::lerp(state_1.twist.linear.z, state_2.twist.linear.z, t_interp);

    interp_state.twist.angular.x = math_utils::lerp(state_1.twist.angular.x, state_2.twist.angular.x, t_interp);
    interp_state.twist.angular.y = math_utils::lerp(state_1.twist.angular.y, state_2.twist.angular.y, t_interp);
                          ...
                          quad_msgs::msg::MultiFootState &interp_state) 
{
    ...
    for (int i = 0; i < interp_state.feet.size(); i++) {
        ...
        interp_state.feet[i].position.x = math_utils::lerp(state_1.feet[i].position.x, state_2.feet[i].position.x, t_interp);
        interp_state.feet[i].position.y = math_utils::lerp(state_1.feet[i].position.y, state_2.feet[i].position.y, t_interp);
        interp_state.feet[i].position.z = math_utils::lerp(state_1.feet[i].position.z, state_2.feet[i].position.z, t_interp);

        // Interp foot velocity
        interp_state.feet[i].velocity.x = math_utils::lerp(state_1.feet[i].velocity.x, state_2.feet[i].velocity.x, t_interp);
        interp_state.feet[i].velocity.y = math_utils::lerp(state_1.feet[i].velocity.y, state_2.feet[i].velocity.y, t_interp);
        interp_state.feet[i].velocity.z = math_utils::lerp(state_1.feet[i].velocity.z, state_2.feet[i].velocity.z, t_interp);

        // Interp foot acceleration
        interp_state.feet[i].acceleration.x = math_utils::lerp(state_1.feet[i].acceleration.x, state_2.feet[i].acceleration.x, t_interp);
        interp_state.feet[i].acceleration.y = math_utils::lerp(state_1.feet[i].acceleration.y, state_2.feet[i].acceleration.y, t_interp);
        interp_state.feet[i].acceleration.z = math_utils::lerp(state_1.feet[i].acceleration.z, state_2.feet[i].acceleration.z, t_interp);
                    ...
                    double t_interp, quad_msgs::msg::GRFArray &interp_state) 
{
    ...
    for (int i = 0; i < interp_state.vectors.size(); i++) {
        interp_state.vectors[i].x = math_utils::lerp(state_1.vectors[i].x, state_2.vectors[i].x, t_interp);
        interp_state.vectors[i].y = math_utils::lerp(state_1.vectors[i].y, state_2.vectors[i].y, t_interp);
        interp_state.vectors[i].z = math_utils::lerp(state_1.vectors[i].z, state_2.vectors[i].z, t_interp);

        interp_state.points[i].x = math_utils::lerp(state_1.points[i].x, state_2.points[i].x, t_interp);
        interp_state.points[i].y = math_utils::lerp(state_1.points[i].y, state_2.points[i].y, t_interp);
        interp_state.points[i].z = math_utils::lerp(state_1.points[i].z, state_2.points[i].z, t_interp);
...
quad_msgs::msg::MultiFootState interpMultiFootPlanContinuous(quad_msgs::msg::MultiFootPlanContinuous msg, double t) 
{
    // Define some useful timing parameters
    // ros::Time t0_ros = msg.states.front().header.stamp;
    // ros::Time t_ros = t0_ros + ros::Duration(t);
...
// quad_msgs::msg::RobotState interpRobotStateTraj(quad_msgs::msg::RobotStateTrajectory
// msg,
//                                            double t) {
//   // Define some useful timing parameters
//   ros::Time t0_ros = msg.states.front().header.stamp;
...
void footStateMsgToEigen(const quad_msgs::msg::FootState &foot_state_msg, Eigen::Vector3d &foot_position) 
{

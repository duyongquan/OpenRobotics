...
void Maps::randomMapGenerate()
{
  ...
  double _w_l = config.width_min;
  double _w_h = config.width_max;
  int    _ObsNum = config.obstacle_number;
...
void Maps::perlin3D()
{
  double complexity = config.complexity;
  double fill = config.fill;
  int    fractal = config.fractal;
  double attenuation = config.attenuation;
...
void Maps::maze2D()
{
  double width = config.road_width;
  int    type = config.maze_type;
  int    addWallX = config.add_wall_x;
  int    addWallY = config.add_wall_y;
...
Maps::Config Maps::getConfig() const
{
  return config;
}

void Maps::setConfig(const Config &value)
{
  config = value;
}
...
void Maps::Maze3DGen()
{
  ...
  double connectivity = config.connectivity;
  int    nodeRad = config.node_rad;
  int    roadRad = config.road_rad;

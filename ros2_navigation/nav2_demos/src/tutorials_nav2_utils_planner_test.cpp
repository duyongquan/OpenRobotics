#include "nav2_demos/tutorials_nav2_utils_planner_test.hpp"
...
void PlannerTester::loadMap(const std::string& pgm_filename)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Loading map with file_path: %s", pgm_filename.c_str());

  try {
    map_ = std::make_shared<nav_msgs::msg::OccupancyGrid>();

    nav2_map_server::LoadParameters load_parameters;
    load_parameters.image_file_name = pgm_filename;
    load_parameters.resolution = resolution;
    load_parameters.origin = origin;
    load_parameters.free_thresh = free_threshold;
    load_parameters.occupied_thresh = occupancy_threshold;
    ...
    RCLCPP_ERROR(
      this->get_logger(),
      "Failed to load image from file: %s", pgm_filename.c_str());
    throw;
  }
...
void PlannerTester::publishPath(const ComputePathToPoseResult & path)
{
  path_publisher_->publish(path);
}

bool PlannerTester::createPlan(
  ...
  ComputePathToPoseResult & path)
{
  ...
  geometry_msgs::msg::Point robot_position;
  robot_position.x = start.pose.position.x;
  robot_position.y = start.pose.position.y;
  updateRobotPosition(robot_position);
  sleep(0.05);
